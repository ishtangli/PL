 ...
 *
 * Added new code 6 => ['AUTOMATED','MSU'] to represent the combined group.
 *
...
function get_location_codes($location): array
{
    $map = [
        ...
        4 => ['INFLAM'],
        5 => ['RAWMAT'],
        6 => ['AUTOMATED', 'MSU'], // NEW: combined AUTOMATED + MSU group
 ...
 *  - DELIVERY_LOCATION
 *  - REQUIRE_ON
 *  - a computed DIV_GROUP:
 *      - MA-LINE when DIVISION starts with MA1
 *      - MA-BASE when DIVISION starts with MA5
 *      - AO when DIVISION starts with AO
 *      - OTHER otherwise
 *
 * TARGET_HOURS are computed based on DIV_GROUP and PRIORITY:
 *  - MA-LINE: AOG = 35 minutes, WSP = 90 minutes, other = 4 hours
 *  - MA-BASE: AOG = 30 minutes, WSP = 90 minutes, other = 8 hours
 *  - AO: target is determined solely by REQUIRE_ON vs CREATED_DATE:
 *        - if TRUNC(CREATED_DATE) = TRUNC(REQUIRE_ON) then target = 1 hour
 *        - otherwise target = end of REQUIRE_ON day (hours remaining in REQUIRE_ON day)
 *  - OTHER: always 8 hours
 *
 * @param int|string $LOCATION
 * @return array
 */
    ...
    try {
        ...
        $sql = "
            SELECT
                ...
                -- Compute TARGET_HOURS (in hours) based on DIV_GROUP and PRIORITY
                CASE
                  WHEN SUBSTR(NVL(rm.DIVISION,''),1,2) = 'AO' THEN
                    -- AO: use REQUIRE_ON to determine target hours (hours, not days)
                    CASE
                      WHEN TRUNC(ph.CREATED_DATE) = TRUNC(ph.REQUIRE_ON) THEN 1
                      ELSE (TRUNC(ph.REQUIRE_ON) + 1 - ph.REQUIRE_ON) * 24
                    END
                  WHEN SUBSTR(NVL(rm.DIVISION,''),1,3) = 'MA1' THEN
                    CASE
                      WHEN ph.PRIORITY = 'AOG' THEN :ml_aog_hours
                      WHEN ph.PRIORITY = 'WSP' THEN :ml_wsp_hours
                      ELSE :ml_other_hours
                    END
                  WHEN SUBSTR(NVL(rm.DIVISION,''),1,3) = 'MA5' THEN
                    CASE
                      WHEN ph.PRIORITY = 'AOG' THEN :mb_aog_hours
                  ...
                  ELSE
                    :other_div_hours
                END AS TARGET_HOURS,
                -- REMAINING_HOURS computed from TARGET_HOURS minus running hours
                (CASE
                  WHEN SUBSTR(NVL(rm.DIVISION,''),1,2) = 'AO' THEN
                    CASE
                      WHEN TRUNC(ph.CREATED_DATE) = TRUNC(ph.REQUIRE_ON) THEN 1
                ...
                -- For AO same-day: CREATED_DATE + 1 hour
                -- For AO different-day: end of REQUIRE_ON day (TRUNC(REQUIRE_ON)+1)
                TO_CHAR(
                  CASE
                    WHEN SUBSTR(NVL(rm.DIVISION,''),1,2) = 'AO' THEN
                      CASE
                    ...
                    ELSE ph.CREATED_DATE + (:other_div_hours)/24
                  END
